...
XLbp = [];
XZernike = [];
salidas = [ones(420,1);2*ones(420,1);3*ones(420,1);4*ones(420,1)];
cont = 0;
time = [];
for(i=3:4:size(FolderList,1)-5)
    AlgaeSegmentedPath = [FolderPath FolderList(i).name '/'];
    AlgaeOriginalPath = AlgaeSegmentedPath
    AlgaeOriginalPath(25:end) = [];
    AlgaeOriginalPath = [AlgaeOriginalPath 'Original/']
    for(j=1:420)
        tic;
        ((420*cont) + j)/1680
        ...
        %Hu moments
        [XHu(((cont*420)+j),:),XnHu] = Bfx_hugeo(segmentedImage); %Hu moments
        %HOG
        options.nj    = 3;                    % 3 x 3
        options.ni    = 3;                    % histograms
        options.B     = 9;                    % 9 bins
        J = rgb2gray(originalImage);
        [XHog(((cont*420)+j),:),XnHog] = Bfx_hog(J,options);    % HOG features (see gradients)
        %Haralick descriptor
        options.dharalick = 3;                                 % 3 pixels distance for coocurrence
        R = segmentedImage;                                    % segmentation
        [XHaralick(((cont*420)+j),:),XnHaralick] = Bfx_haralick(J,R,options);    % Haralick features
        %LBP descriptor
        ...
        [XLbp(((cont*420)+j),:),XnLbp] = Bfx_lbp(J,[],options);    % LBP features
        %Zernike moments
